(ns i2conomy.mint
  (:use [i2conomy.validation :only [defvalidation validate]])
...
(defn balance
  ...
  [username currency]
    (validate [:account-exists username])
...
(defn create-account
  ...
  ([username password]
  (dosync
    (validate [:account-available username])
...
(defn pay
  ...
  (dosync
    (validate [:account-exists from] [:account-exists to] [:currency-exists currency]
